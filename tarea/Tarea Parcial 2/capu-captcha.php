...
for ($i = 1; $i < 20; $i++) {
    $x = mt_rand(0, 150); // pos x
    $y = mt_rand(0, 50); // pos y
    $car = chr(mt_rand(45, 250)); //caracter aleatorio
    $angulo = mt_rand(3, 180);
    imagettftext($imagen, 8, $angulo, $x, $y, $color_basura, $randFontPath, $car);
}

// Imprimir la palabra
$x = mt_rand(10, 30);
$y = mt_rand(20, 35);
imagettftext($imagen, 16, mt_rand(-10, 10), $x, $y, $color_texto, $randFontPath, $palabra);

// Ensuciamos la imagen con líneas
for ($i = 1; $i < 30; $i++) {
    $x = mt_rand(0, 150);
    $y = mt_rand(0, 50);
    imageline($imagen, $x, $y, $x + mt_rand(-20, 20), $y + mt_rand(-20, 20), $color_basura);
}
